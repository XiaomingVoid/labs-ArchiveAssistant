...
internal class MemorialFoldView(context: Context) : View(context) {
    ...
    private fun drawSummaryBubble(canvas: Canvas, coverLeft: Float, cover: ArticleLayout) {
        ...
        val summaryTitlePaint = TextPaint(paints.itemTitle).apply {
            textSize = sp(26f)
            textAlign = Paint.Align.LEFT
            color = MEMORIAL_INK_BROWN
        }
        val sourcePaint = TextPaint(paints.itemMeta).apply {
            textSize = sp(18f)
            textAlign = Paint.Align.LEFT
            color = IMPERIAL_GOLD_DARK
        }
        val bodyPaint = TextPaint(paints.author).apply {
            textSize = sp(20f)
            textAlign = Paint.Align.LEFT
            color = MEMORIAL_INK_BROWN
        }
        ...
        val titleLayout = buildEllipsizedTextLayout(summary.title, summaryTitlePaint, contentWidth, 1.18f, 1)
    ...
    private fun drawCompletedState(canvas: Canvas, alpha: Float) {
        ...
        val completionTitlePaint = TextPaint(paints.title).apply {
            textSize = sp(33f)
            letterSpacing = 0.08f
        }
        val completionBodyPaint = TextPaint(paints.quote).apply {
            textSize = sp(22f)
            color = AndroidColor.rgb(72, 43, 31)
            textAlign = Paint.Align.LEFT
        }
        val completionHintPaint = TextPaint(paints.meta).apply {
            textSize = sp(21f)
            color = IMPERIAL_GOLD_DARK
        }
    ...
    private fun drawToolbarButton(canvas: Canvas, rect: RectF, label: String) {
        ...
        drawCenteredText(canvas, label, rect.centerX(), rect.centerY() + textCenterOffset(paints.toolbarText), paints.toolbarText)
    }
    ...
    private fun drawRubberStamp(
        ...
        paints.stampText.textSize = when (stamp) {
    ...
    private fun drawVerticalStampText(
        ...
        val baselineOffset = -(paints.stampText.fontMetrics.ascent + paints.stampText.fontMetrics.descent) / 2f
    ...
    private fun drawCoverBackground(canvas: Canvas, rect: RectF, coverSequenceIndex: Int) {
        ...
        paints.surface.shader = LinearGradient(
            rect.left,
            rect.top,
            rect.right,
            rect.bottom,
    ...
    private fun drawBitmapCenterCrop(canvas: Canvas, bitmap: android.graphics.Bitmap, dst: RectF, paint: Paint) {
    ...
    private fun drawPaperAging(canvas: Canvas, rect: RectF, pageIndex: Int) {
        paints.paperAging.shader = LinearGradient(
            rect.left,
            rect.top,
            rect.right,
            rect.bottom,
    ...
    private fun drawDynamicFoldShadow(
        ...
        paints.foldShadow.shader = LinearGradient(
            shadowLeft,
            0f,
            shadowRight,
            0f,
    ...
    private fun drawCoverCornerOrnament(canvas: Canvas, texture: android.graphics.Bitmap, dst: RectF, rotation: Float) {
        canvas.save()
    ...
    private fun drawDirectoryContent(canvas: Canvas, rect: RectF, pageNumber: String) {
        ...
        directoryItems.forEach { item ->
            ...
            y += dp(28f)
            val excerptLayout = buildTextLayout(item.excerpt, paints.author, itemWidth.roundToInt(), 1.7f, false)
            canvas.save()
    ...
    private fun drawBodyRightContent(canvas: Canvas, rect: RectF, pageNumber: String) {
        ...
        y = box.bottom + dp(46f)
        val signPaint = TextPaint(paints.author).apply {
    ...
    private fun drawEndContent(canvas: Canvas, rect: RectF, pageNumber: String) {
        ...
        val endPaint = TextPaint(paints.title).apply {
            textSize = sp(42f)
            letterSpacing = 0.16f
        }
        ...
        drawCenteredText(canvas, "御览", seal.centerX(), seal.centerY() + textCenterOffset(paints.sealText), paints.sealText)
        canvas.restore()
    ...
    private fun drawArticleSurface(canvas: Canvas, rect: RectF) {
        ...
        canvas.drawLine(rect.right - 1f, rect.top + dp(10f), rect.right - 1f, rect.bottom - dp(10f), paints.crease)
