...
class ArchiveAssistantStateStore(
  ...
  var state: ArchiveAssistantState by mutableStateOf(resolveMockImagePaths(initialState))
    private set
  ...
  private fun shouldRefreshDemoData(
    ...
    val looksLikeOldDemoData =
      persistedTopics.map { it.id }.toSet().containsAll(oldDemoTopicIds) &&
        persistedTopics.size <= oldDemoTopicIds.size &&
        persistedItems.size <= 8
    val looksLikeCurrentDemoData =
      persistedTopics.map { it.id }.toSet().containsAll(SampleKnowledgeData.topics.map { it.id }) &&
  ...
  private fun resolveMockImagePaths(state: ArchiveAssistantState): ArchiveAssistantState {
    ...
    val mapping = mapOf("item-transformer-diagram" to "transformer_architecture")
  ...
  private suspend fun createSmartSummarizeRequest(rawInput: String): Result<SmartSummarizeRequest> {
    ...
    val detected =
      WebUrlDetector.detect(rawInput)
    ...
    state =
      state.copy(
        topics =
          state.topics.map { topic ->
            if (topic.id == topicId)
              topic.copy(title = normalizedTitle, updatedAtEpochMillis = renameEpoch)
            else topic
          },
        topicValidationMessage = null,
      )
  ...
  fun acceptClipboardAndManualCreate() {
    val targetTopicId =
      state.selectedTopicId?.takeIf { selectedTopicId ->
